...
def tic_tac_toe(board):
    ...
    if len(winner_board) == 1:
        return str(winner_board)[2]
    else:
        return "NO WINNER"
        ...
        while current_stop != second_stop:
            for key in route_map.keys():
                if current_stop == key[0]:
                    start_travel = key
            for key in route_map.keys():
                if second_stop ==key[1]:
                    end_travel = key
            total_time.append(route_map[start_travel]["travel_time_mins"])
            current_stop = start_travel[1]
